...
import { NO_OF_QUESTIONS } from './constants';
...
export async function createNewGame(formData) {
  ...
  if (!questions || questions.length !== NO_OF_QUESTIONS) {
...
export async function submitAnswer({ roomCode, playerName, answer }) {
  ...
  if (allAnswered) {
    ...
    //   if (blackSheep) blackSheep.isBlackSheep = true;
    // }
    ...
    if (uniqueAnswers.length === 1) {
      ...
      // Remove Black Sheep status from all players
      gameState.players.forEach(player => {
        player.isBlackSheep = false;
      });

      // Assign Black Sheep status to the new Black Sheep
      const newBlackSheep = gameState.players.find(p => p.id === blackSheepId);
      if (newBlackSheep) {
        newBlackSheep.isBlackSheep = true;
      }
    } 
    // else {
    //   // If there's no unique answer, ensure no one has Black Sheep status
    //   gameState.players.forEach(player => {
    //     player.isBlackSheep = false;
    //   });
    // }
